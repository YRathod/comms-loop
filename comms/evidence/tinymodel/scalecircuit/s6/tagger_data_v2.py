...
def make_chain(rng, max_depth=4):
    depth = rng.choices([1, 2, 3, 4], weights=[35, 40, 20, 5])[0]
    rels = [rng.choice(list(RELS)) for _ in range(depth)]
    oov_slot = rng.randrange(depth) if rng.random() < 0.15 else None    # (b) one relation phrased outside the set -> attribute
    if oov_slot is not None:
        rels[oov_slot] = "attribute"
    ent = rng.choice(DESCRIPTORS) if rng.random() < 0.10 else entity(rng)   # (d) descriptor anchors
    phrase = ent
    for i, r in enumerate(rels[:-1]):                                   # inner hops as nominal phrases, innermost first
        form = rng.choice(OOV_NOMINAL) if i == oov_slot else rng.choice(RELS[r][0])
    ...
    elif r < 0.45:                                                      # (c) a modifier that is NOT a filter
...
def make_na(rng):
    ...
    q = rng.choice([f"Which came first, {a} or {b}?", f"Which has more species, {a} or {b}?", f"Are {a} and {b} both located in the same state?",
                    ...
                    f"How many films did {a} and {b} make together?"])
...
def main():
    ...
    with open(args.out, "w", encoding="utf-8") as f:
        for _ in range(args.n):
            q, t = make_na(rng) if rng.random() < 0.12 else make_chain(rng)
