...
def eval_real(model, tok, device, verbose=True):
    out = []
    ...
    print("real 30:", summary, flush=True)
    return summary, out
...
def main():
    ...
    if args.eval_only:
        ...
        eval_synth(model, tok, device, held[:200]); eval_real(model, tok, device)
        return
    ...
    while step < total:
        ...
        for enc, labels in batches(tok, pairs, args.bs, device):
            ...
            step += 1
            if step % 25 == 0:
                print(f"step {step}/{total} loss {out.loss.item():.4f} {time.time() - t0:.0f}s", flush=True)
            if step >= total:
                break
    ...
    summary, rows = eval_real(model, tok, device)
