...
DEFAULT_DATA = r"C:\dev\research\memory-challange\long-bench\data\data\hotpotqa.jsonl"
ALL_ROWS = None   # loaded in main() from --data (LongBench-format jsonl: input / context / answers)
...
def main():
    ...
    ap.add_argument("--data", default=DEFAULT_DATA, help="LongBench-format jsonl; default = the LongBench hotpotqa file (docs 0-199)")
    args = ap.parse_args()
    global ALL_ROWS
    ALL_ROWS = [json.loads(l) for l in open(args.data, encoding="utf-8")]
    ...
    docs = [i for i in range(lo, hi + 1) if not (i in FEWSHOT and hi <= 39 and args.data == DEFAULT_DATA)]     # few-shot docs excluded only in the LongBench eval range
    tags = {}
    for i in docs:
    ...
    for i, tag in tags.items():
        ...
        if hops:
            ...
            itf = sp if (verbose(it) and not verbose(sp)) else it                 # claimed wiring (s15): iterative + gold-free shape fallback
        else:
            ch, rt, it, itn, su, sun, suf, sel, itf, how, trace, itrace = sp, sp, sp, sp, sp, sp, sp, sp, sp, "single-pass", [("single-pass fallback (NA or unusable tag)", sp, "")], []
        row = {"doc": i, "q": r["input"], "gold": r["answers"], "tag": tag, "key": KEY.get(i), "hops": hops, "entity": ent,
               ...
               "superset": su, "superset_notes": sun, "superset_fallback": suf, "select": sel, "select_how": how, "f1_select": f1(sel, r["answers"]), "iterative_fallback": itf, "f1_iterative_fallback": f1(itf, r["answers"]),
        ...
        res.append(row)
        print(f"doc {i:3d} sp {row['f1_sp']:.2f} chain {row['f1_chain']:.2f} retr {row['f1_retrieval']:.2f} iter {row['f1_iterative']:.2f} iter+n {row['f1_iterative_notes']:.2f} sup {row['f1_superset']:.2f} sup+n {row['f1_superset_notes']:.2f} sup+fb {row['f1_superset_fallback']:.2f} sel {row['f1_select']:.2f}({how[:5]}) ITF {row['f1_iterative_fallback']:.2f} | tag {tag[:30]!r} | {sp[:20]!r} / {su[:20]!r} | gold {r['answers'][0][:20]!r}", flush=True)
    n = len(res)
    W = ("f1_sp", "f1_chain", "f1_retrieval", "f1_iterative", "f1_iterative_notes", "f1_superset", "f1_superset_notes", "f1_superset_fallback", "f1_select", "f1_iterative_fallback")
