...
def eval_slice(adapter, pairs):
    ...
    # ok = structure + anchor + filter right (the s3 band); relation-token mismatches are autopsied but
    # not counted as failures, because the key's relation tokens are coarse stand-ins
    ok = sum(c in (None, "relation") for c in classes)
    buckets = {}
    for c in classes:
        if c:
...
ROUTE = {  # dominant failure class -> generator knobs (see tagger_data.py main() env overrides)
    "hops_dropped": {"TAGGER_DEPTH_WEIGHTS": "20,35,30,15"},
    "hops_extra": {"TAGGER_OOV_FRAC": "0.05"},
    "anchor": {"TAGGER_DESC_FRAC": "0.25"},
...
def main():
    ...
    flat = 0
    for r, batch_path in enumerate(args.batches[: args.max_rounds], start=1):
